...
- `capabilities/music/src/player.rs` supervisor loop: if no `credentials.json` → warn + sleep 60 s; resolve `paired_bluetooth_sink()` fresh each spawn; spawn `librespot --name <SPOTIFY_DEVICE_NAME> --backend pipe --cache ~/.ai-mesh/spotify-cache --format S16 --bitrate 160 --initial-volume 60` (stdout piped) → `pacat --device=<sink> --raw --format=s16le --rate=44100 --channels=2` (stdin piped) → `tokio::io::copy` between them; on either exit, kill both, backoff 2 s→60 s (reset after 5 min healthy). First spawn: `pkill -f "librespot --name <name>"` to reap orphans from `systemctl kill` deploys (precedent: stt.rs port-based stray-kill).
