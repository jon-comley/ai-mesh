...
- **Mac Studio M4 Max (`mac1`)** — **the hardware is here and running as of 2026-09-09**, with an iPhone attached to it. Not on the mesh yet, and the line below used to say the wrong thing about how to get it there.
  ...
  **`mac1` is `jonno@192.168.1.22`** — `Jonathans-Mac-Studio`, macOS 26.5 (25F71), `Mac16,9`, arm64, 64GB, 1.8TB free. Remote Login is on and sshd listens on tcp4 and tcp6. It is on Wi-Fi (`en1`); `en0` has no address, so the 10Gb Ethernet is unplugged and the address is a DHCP lease against a **randomised** Wi-Fi MAC (`2a:86:fb:fe:e9:80`) — it can move.

  ⚠ **This box still cannot reach the Mac directly, and that is not the Mac's fault.** No ICMP and no TCP/22 from here, while `pi1` reaches it fine over the same Wi-Fi: `ping` 3/3, port 22 open, banner `SSH-2.0-OpenSSH_10.2`, ARP matching the MAC above. Both sit on the same Orbi network and the Mac's own firewall is **disabled**, so the drop is in the network rather than on either host. Not diagnosed further because it does not have to be: `~/.ssh/config` on this box defines `Host mac1` with `ProxyJump pi1`, and that works. **Anything written here that assumes a direct path from this machine to the Mac will fail** — go through `pi1`.
