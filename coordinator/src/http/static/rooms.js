...
// below are drag gestures that only rooms.js touches. The effects domain (its
// drag source + open-editor room) lives in effects.js, sharing state via the
// state.effectDrag / state.effectEditor holders.
...
fetchDeviceNames();
// Wire the effects module's render dependency before its first catalogue fetch
// (and before any WS EffectUpdate) so its optimistic re-renders work. render is
// a hoisted function declaration, so the reference is live at this point.
initEffects({ render });
fetchEffectsCatalog();
