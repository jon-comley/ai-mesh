...
// ── Wall photos ───────────────────────────────────────────────────────────────
// Phase 4: a manual-tracing aid for this room's dimensions/orientation/
// opening-placement flow, not room scanning — see the schema comment on
// `room_wall_photos` in registry/mod.rs. Fetched lazily by the layout editor
// (not part of RoomInfo/the WS snapshot — a photo can be a few hundred KB and
// has no business riding every unrelated room-membership broadcast).

/// Generous headroom over what the client actually sends: layout.js
/// downscales to a modest resolution before base64-encoding, so a legitimate
/// upload should land well under this; it exists to reject pathological
/// payloads, not to be a tight budget. `pub(crate)` so `http::mod` can size
/// its `DefaultBodyLimit` layer to match.
pub(crate) const MAX_WALL_PHOTO_DATA_URI_LEN: usize = 8 * 1024 * 1024;

pub async fn get_wall_photos(
    Path(room_id): Path<String>,
    Extension(registry): Extension<Arc<Mutex<Registry>>>,
    _: Authed,
) -> impl IntoResponse {
    let photos = registry.lock().unwrap().get_wall_photos(&room_id);
    let map: serde_json::Map<String, serde_json::Value> = photos
        .into_iter()
        .map(|(edge, uri)| (edge, serde_json::Value::String(uri)))
        .collect();
    Json(serde_json::Value::Object(map)).into_response()
}

#[derive(Deserialize)]
pub struct SetWallPhotoBody {
    data_uri: String,
}

pub async fn set_wall_photo(
    Path((room_id, wall_edge)): Path<(String, String)>,
    Extension(registry): Extension<Arc<Mutex<Registry>>>,
    _: Authed,
    Json(body): Json<SetWallPhotoBody>,
) -> impl IntoResponse {
    if !VALID_WALL_EDGES.contains(&wall_edge.as_str()) {
        return (StatusCode::BAD_REQUEST, "wall_edge must be N, S, E, or W").into_response();
    }
    if body.data_uri.len() > MAX_WALL_PHOTO_DATA_URI_LEN {
        return (StatusCode::BAD_REQUEST, "photo too large").into_response();
    }
    {
        let reg = registry.lock().unwrap();
        if !reg.room_exists(&room_id) {
    ...
    registry
        .lock()
        .unwrap()
        .set_wall_photo(&room_id, &wall_edge, &body.data_uri);
    StatusCode::NO_CONTENT.into_response()
}

pub async fn delete_wall_photo(
    Path((room_id, wall_edge)): Path<(String, String)>,
    Extension(registry): Extension<Arc<Mutex<Registry>>>,
    _: Authed,
) -> impl IntoResponse {
    let mut reg = registry.lock().unwrap();
    if !reg.wall_photo_exists(&room_id, &wall_edge) {
        return StatusCode::NOT_FOUND.into_response();
    }
    reg.delete_wall_photo(&room_id, &wall_edge);
    StatusCode::NO_CONTENT.into_response()
}
...
mod tests {
    ...
    fn wall_photos_router(state: Arc<DashboardState>, registry: Arc<Mutex<Registry>>) -> Router {
        Router::new()
            .route("/api/rooms/{id}/wall-photos", get(get_wall_photos))
            .route(
                "/api/rooms/{id}/wall-photos/{wall_edge}",
                axum::routing::put(set_wall_photo).delete(delete_wall_photo),
            )
            .layer(axum::Extension(registry))
            // Matches the raised limit http::mod applies in the real router —
            // otherwise axum's default 2MB extractor limit would reject an
            // oversized test payload with a generic 413 before our own
            // length check (the thing under test) ever runs.
            .layer(axum::extract::DefaultBodyLimit::max(
                MAX_WALL_PHOTO_DATA_URI_LEN + 4096,
            ))
            .with_state(state)
    }

    #[tokio::test]
    async fn set_wall_photo_returns_204_then_get_returns_it() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        let state = make_state(vec![], empty_connections());
        let status = send(
            wall_photos_router(state.clone(), Arc::clone(&registry)),
            "PUT",
            &format!("/api/rooms/{room_id}/wall-photos/N?token="),
            r#"{"data_uri":"data:image/jpeg;base64,abc"}"#,
        )
        .await;
        assert_eq!(status, StatusCode::NO_CONTENT);

        let (status, body) = send_with_body(
            wall_photos_router(state, registry),
            "GET",
            &format!("/api/rooms/{room_id}/wall-photos?token="),
            "",
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        assert!(body.contains("data:image/jpeg;base64,abc"), "body: {body}");
    }

    #[tokio::test]
    async fn set_wall_photo_returns_400_for_invalid_wall_edge() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        let state = make_state(vec![], empty_connections());
        let status = send(
            wall_photos_router(state, registry),
            "PUT",
            &format!("/api/rooms/{room_id}/wall-photos/C?token="),
            r#"{"data_uri":"data:image/jpeg;base64,abc"}"#,
        )
        .await;
        assert_eq!(
            status,
            StatusCode::BAD_REQUEST,
            "the ceiling sentinel isn't a wall — no photo slot for it"
        );
    }

    #[tokio::test]
    async fn set_wall_photo_returns_404_for_unknown_room() {
        let registry = make_registry();
        let state = make_state(vec![], empty_connections());
        let status = send(
            wall_photos_router(state, registry),
            "PUT",
            "/api/rooms/ghost/wall-photos/N?token=",
            r#"{"data_uri":"data:image/jpeg;base64,abc"}"#,
        )
        .await;
        assert_eq!(status, StatusCode::NOT_FOUND);
    ...
    #[tokio::test]
    async fn set_wall_photo_returns_400_for_oversized_payload() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        let state = make_state(vec![], empty_connections());
        let huge = "a".repeat(MAX_WALL_PHOTO_DATA_URI_LEN + 1);
        let status = send(
            wall_photos_router(state, registry),
            "PUT",
            &format!("/api/rooms/{room_id}/wall-photos/N?token="),
            &format!(r#"{{"data_uri":"{huge}"}}"#),
        )
        .await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
    }
    ...
    #[tokio::test]
    async fn get_wall_photos_returns_empty_object_when_none_set() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        let state = make_state(vec![], empty_connections());
        let (status, body) = send_with_body(
    ...
    #[tokio::test]
    async fn delete_wall_photo_returns_204_and_removes_it() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        registry
            .lock()
            .unwrap()
            .set_wall_photo(&room_id, "N", "data:image/jpeg;base64,abc");
        let state = make_state(vec![], empty_connections());
        let status = send(
            wall_photos_router(state.clone(), Arc::clone(&registry)),
            "DELETE",
        ...
        .await;
        assert_eq!(status, StatusCode::NO_CONTENT);
        assert!(!registry.lock().unwrap().wall_photo_exists(&room_id, "N"));
    }

    #[tokio::test]
    async fn delete_wall_photo_returns_404_for_unknown_photo() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        let state = make_state(vec![], empty_connections());
        let status = send(
    ...
    #[tokio::test]
    async fn wall_photo_routes_return_401_for_wrong_token() {
        let registry = make_registry();
        let room_id = make_room(&registry, "Kitchen");
        let state = make_state(vec!["secret".into()], empty_connections());
        let status = send(
            wall_photos_router(state, registry),
            "PUT",
            &format!("/api/rooms/{room_id}/wall-photos/N?token=wrong"),
            r#"{"data_uri":"data:image/jpeg;base64,abc"}"#,
        )
        .await;
        assert_eq!(status, StatusCode::UNAUTHORIZED);
