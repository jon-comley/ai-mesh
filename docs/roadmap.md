...
- **F-Effects ✓ (palette UI)** — Effects/Scenes taxonomy and drag-from-palette UX. Establishes a clear design split:
  ...
  - **Effects palette** — a draggable chip strip above the room list. Chips are categorised (dynamic effects vs. scenes). Drag a chip onto any room card → enables that effect for all room devices. The room card gains a coloured badge (e.g., ☀ Solar in amber) that confirms the effect is active; clicking the badge disables it.
  ...
  - **Engineering note — effect handler registry**: The current drop handler uses a single `if (effect === 'solar')` check. When the second effect lands (Circadian or Telemetry), replace with a handler map: `const effectHandlers = { solar: (roomId) => setSolarMode(roomId, true), circadian: ..., telemetry: ... }; effectHandlers[effect]?.(roomId);` — avoids a growing switch statement and keeps each effect self-contained.
  - **Engineering note — optimistic UI**: `setSolarMode` patches `devicesMap` before the server confirms. The coordinator broadcasts a `LightingUpdate` on `set_solar_enabled`, so the WS event auto-corrects within milliseconds regardless of success or failure. No manual rollback needed.
...
- **F-Spatial-2** — Room Layout Canvas. Clicking a room name/header opens a full layout view for that room (replaces the card in-place, back button returns to the room list). The layout view is an SVG canvas — a top-down floor plan the user builds themselves.
  ...
  - **Phase A — Bulb placement**: SVG canvas fills the view. Bulb chips from the room's device list appear in a sidebar; drag one onto the canvas and it drops to position, pulsing in real life as you drag (pulse-on-grab already works). Position stored as 0–1 normalised (x, y, z) floats via existing `POST /api/lights/{device}/position` — the `z` column already exists in the `light_positions` schema but was previously unused. `SpatialEngine` immediately uses updated coordinates on next sweep.
    - **Light type classification** — each bulb has a `fixture_type` field (`ceiling_spot` | `pendant` | `table_lamp` | `floor_lamp` | `led_strip`). Type is set via a small picker on the sidebar chip or canvas popover; it drives three things: (1) the default Z height assigned on first placement, (2) the icon rendered on the canvas floor plan, and (3) the solar sensitivity weighting. Default Z by type: ceiling spot → 1.0, pendant → 0.6, table lamp → 0.3, floor lamp → 0.1, LED strip → wall/user-defined.

      | Type | Default Z | Canvas icon | Solar sensitivity |
      |---|---|---|---|
      | Ceiling spot | 1.0 | filled circle | low — sits above the window path |
      | Pendant | 0.6 | circle + drop line | medium |
      | Table lamp | 0.3 | circle + base | high — at window sill height |
      | Floor lamp | 0.1 | circle + tall base | medium-low |
      | LED strip | user-set | bar segment | varies by mount height |

    - **Height (Z) control** — after placing a bulb, a small height slider appears in the canvas popover (floor → ceiling). Default is set from the fixture type but fully overridable. Z is shown as a subtle vertical offset on the canvas icon (pendants draw a drop line whose length reflects Z; ceiling spots are flat circles).
    - **Invisible snap grid** — bulbs snap to a soft invisible grid on drop; default cell size is 1/20 of the canvas but the user can pinch or use a ± control to adjust the granularity. No grid lines shown — placement just feels tidy.
    - **Placement preview** — as the bulb chip is dragged over the canvas, a subtle glowing circle (matching the bulb's current colour) marks the snapped landing spot before the user releases. Combined with pulse-on-grab (bulb already flashing in the room), this creates a live link between the physical light and its canvas position. Snap targets prioritise window centres and wall midpoints so bulbs align naturally with the room geometry without the user having to be precise.
    - **Undo/redo** — every placement, move, or delete is a reversible step (state snapshot stack in JS; no server round-trips for undo).
  - **Phase B — Windows & Doors**: `openings` SQLite table (`room_id`, `type` TEXT — `"window"` | `"door"`, `wall_edge` NSEW, `x_norm` 0–1 position along that wall, `width_norm` 0–1, `transmission` 0–1). Multiple openings per room. In the canvas, user drags a window or door chip from the sidebar toward any wall; the element **magnetically snaps** to the nearest wall edge when within ~60px, auto-assigning its facing. Drag handles on either end resize it in-place — direct manipulation, no presets.
    - **Transmission is always user-settable** — `type` drives visuals and future semantics only. A **window** defaults `transmission` to 0.8. A **door** can be solid (transmission 0.0) or glass (transmission 0.6) — glass doors contribute to solar just like windows. `SpatialEngine` reads `transmission` directly, ignoring `type`; the swing-arc glyph is rendered for all door types regardless of glass or solid.
    - **Auto-shadow preview** — immediately after placing or moving an opening, the canvas renders a soft inward light cone at the angle matching the current solar azimuth, with opacity scaled by `transmission`. This is a client-side SVG overlay only — it teaches the user why solar mode behaves the way it does without running the real engine.
    - **Door semantics (future)** — door positions and adjacency are stored and available for F7 presence detection (motion sensor → room entry events), Hot/Cold game routing, and "follow-me" lighting across adjacent rooms.
    - Replaces the current flat `has_window`/`window_facing` columns. `SpatialEngine` iterates all openings and takes the max weighted intensity across all windows (a bulb lit by two windows is brighter than one lit by one).
  ...
  - **Phase D — Live sun arc**: A sun icon traces an arc across the top of the canvas showing today's solar path; a pulsing dot marks the current azimuth (driven by the existing `SolarUpdate` WS event, no new backend needed). A scrub slider lets the user preview "what will my lights look like at 4pm?" — dragging moves the sun, bulbs animate to their preview state; releasing returns to real-time mode. Makes the SpatialEngine logic tangible and child-friendly.
