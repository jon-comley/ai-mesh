...
- **F-Quality ✓** — Lighting stability, colour intents, and operational reliability.
  ...
  - **Colour intents** — `light_command` tool schema gains a `"color"` action with a `"color"` string parameter (CSS names or hex). `build_light_command` maps named colours (red, green, blue, orange… 18 total) and hex strings (#RRGGBB / #RGB) to `LightAction::ColorXY` via `css_color_to_xy` + `rgb_to_xy` (inverse sRGB gamma + wide-gamut D65 matrix). `just intent "make test bulb green"` now sends `ColorXY` instead of the old `color_temp: 7000`. 11 new tests.
...
- **F8** — Creative Features. See *The Innovations* above. Delivery order: (1) Telemetry Lighting; (2) Photo Colour Picker (client-side Canvas API); (3) Intent-First Scenes; (4) Temporal Composer; (5) Reactive Graph; (6) 2D Room Layout (SVG floor plan, builds on F-Rooms + F-Spatial).
