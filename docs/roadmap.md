...
- **F-Effects-2 (Planned)** — Effects registry + curated catalogue. Full design spec: `plans/phase11f-effects-2.md`. Replaces the placeholder `rooms.solar_enabled` bool and inline `if (effect === 'solar')` drop-handler with an extensible registry. Adding effect #N becomes a single file in `coordinator/src/effects/`.
  - **`Effect` trait** with `id`, `display_name`, `category`, `cadence`, `params_schema`, `default_params`, `tick(ctx) -> Vec<LightCommand>`, lifecycle hooks (`on_enable`, `on_handoff`, `on_disable`), `respects_overrides`, and an opt-in persistence triad (`persist_cadence`, `serialize_internal_state`, `deserialize_internal_state`) so stochastic effects survive a coordinator restart without continuous disk writes.
  - **`SpatialHelpers`** — single home for the geometric questions every effect asks: `west_to_east`, `angle_to_sun`, `window_proximity`, `altitude_band`, `distance_to_wall`, `directional_offset`. Effects become pure functions over spatial context.
  - **`room_effects` SQLite table** with `snapshot_json` + `internal_state_json` + partial unique index enforcing one-active-effect-per-room at the DB layer + `CHECK (enabled IN (0,1))`. Silent migration from `rooms.solar_enabled`.
  - **`EffectRunner`** — tick scheduler + Last Emitted State cache (per-room, per-device LES recording resolved values actually dispatched) + delta-threshold dedup gate (brightness Δ≥2, xy Δ≥0.005, CT Δ≥4 mireds) + two-stage OKLCH interpolation for effect→effect handoff (per-property normalisation then perceptual blend) + cadence drift EWMA. LES seeded from `light_states` on restart.
  - **Interpolation layer pays off twice**: same `BlendPoint` + OKLCH machinery reusable for time scrubbing, parameter preview sliders, jump-to-midpoint controls, and arbitrary-duration scene recall. Keep `blend(A, B, t)` public crate-wide, not buried in `effects/`.
  - **Initial catalogue** — Solar (port), Sunset, Sunrise, Candlelight, Aurora, Breathing. Each chosen to exercise a different `SpatialHelpers` method so the helper API is shaken out before the registry closes. Sunset is position-aware (west-biased via `directional_offset`, fixture-type personality); Candlelight uses per-bulb seeds derived from a single persisted `master_seed` via Xoshiro `advance(step)` (O(log n) resume cost regardless of elapsed time).
  - **Data-driven frontend** — palette renders from `GET /api/effects`; generic `<EffectParamsEditor>` reads each effect's JSON Schema and renders sliders/segmented buttons with `min`/`max`/`step` enforced at the input layer (server `jsonschema` validation is the backstop). When the catalogue grows past ~8, group chips under the existing `EffectCategory` enum as tabs.
  - **17 specific unit tests** — resume after restart (time-based and seed-based), handoff (no flicker, snapshot transferred verbatim), dedup thresholds, OKLCH red→blue acid test (passes through clean purple not muddy grey), mismatched-mode CT↔xy interpolation, missing-bulb carry-over, override respect (true and false), cadence drift warning, param validation rejection, partial unique index enforcement.
  - **Sub-phases**: 2.1 registry + runtime plumbing → 2.2 discovery/activation API → 2.3 frontend palette + param editor → 2.4 Sunset → 2.5 Sunrise + Breathing + Candlelight → 2.6 Aurora + Telemetry skeleton → 2.7 cleanup (drop `rooms.solar_enabled`).
  - **Engineering decisions locked**: single effect per room for MVP (Telemetry overlay revisited later); tick-driven (not reactive streams); JSON Schema params (no per-effect frontend code); seed+elapsed persistence pattern preferred over continuous writes.
