...
- **F-Spatial-2** — Room Layout Canvas. Clicking a room name/header opens a full layout view for that room (replaces the card in-place, back button returns to the room list). The layout view is an SVG canvas — a top-down floor plan the user builds themselves.
  ...
  - **Phase A — Bulb placement**: SVG canvas fills the view. Bulb chips from the room's device list appear in a sidebar; drag one onto the canvas and it drops to position, pulsing in real life as you drag (pulse-on-grab already works). Position stored as 0–1 normalised (x, y) floats via existing `POST /api/lights/{device}/position`. `SpatialEngine` immediately uses updated coordinates on next sweep.
  - **Phase B — Windows**: `windows` SQLite table (`room_id`, `wall_edge` NSEW, `size_factor` 0–1, `transmission` 0–1). Multiple windows per room. In the canvas, user drags a window rectangle to any wall edge; facing auto-assigned from the edge. Replaces the current flat `has_window`/`window_facing` columns. `SpatialEngine` iterates all windows and takes the max weighted intensity (a bulb lit by two windows is brighter than one lit by one).
  - **Phase C — Live sun arc**: A sun icon traces an arc across the top of the canvas showing today's solar path; a pulsing dot marks the current azimuth (driven by the existing `SolarUpdate` WS event, no new backend needed). A scrub slider lets the user preview "what will my lights look like at 4pm?" — making the SpatialEngine logic tangible and child-friendly.
  - **Engineering**: Room layout view is a new `layout.js` module. Canvas coordinates are always 0–1 so the SVG scales to any screen. Bulb icons on canvas reflect live state (colour swatch, brightness) from the existing `devicesMap`.
