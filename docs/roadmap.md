...
- **F-Spatial-2** — Room Layout Canvas. Clicking a room name/header opens a full layout view for that room (replaces the card in-place, back button returns to the room list). The layout view is an SVG canvas — a top-down floor plan the user builds themselves.
  ...
  - **Phase C — Live sun arc**: A sun icon traces an arc across the top of the canvas showing today's solar path; a pulsing dot marks the current azimuth (driven by the existing `SolarUpdate` WS event, no new backend needed). A scrub slider lets the user preview "what will my lights look like at 4pm?" — dragging moves the sun, bulbs animate to their preview state; releasing returns to real-time mode. Makes the SpatialEngine logic tangible and child-friendly.
