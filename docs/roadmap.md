...
- **F-Rooms** — First-class user-managed rooms. Full spec: `plans/phase11f-rooms.md`. Rooms are stored in the coordinator's own SQLite — independent of Z2M groups, portable across device types. Drag-and-drop UX: devices live in an "Unassigned" strip until dragged into a room card. Room cards have on/off, brightness, CT controls that fan out to all member devices. This is the UI Hue should have built.
  ...
  - **F-Rooms-3** ← *next* — `rooms.js` ES module. `RoomsUpdate` handler drives a full re-render on every WS event. Layout: `[ + New Room ]` button at top; **Unassigned** strip (devices with no room membership shown as draggable chips); **room cards** (name, on/off buttons, brightness slider, CT slider, member device chips with `✕` remove button, drop-zone highlight). Drag-and-drop: HTML5 DnD reusing existing engine — `dragstart` records `{ deviceId, fromRoomId }`, `drop` calls `PATCH /api/rooms/{id}/devices`. Inline rename: click name → `<input>` in place, Enter/blur confirms, Escape cancels. `+ New Room` inline input. `dashboard.js` routes `RoomsUpdate` to `rooms.js`; `lighting.js` flat device list suppressed when rooms module is active. New `/static/rooms.js` route in `mod.rs`. CSS: room card, device chip, Unassigned strip, drag-over highlight.
  - **F-Rooms-4** — Polish: compact device chips with tap-to-expand full controls inline; drag-to-reorder rooms (`position` column already in DB); room-level colour picker (XY).
- **F6** — Scenes (basic). Snapshot current full state → save as a named scene → one-tap recall. `POST /api/scenes/save { name, room? }` snapshots the current state; `POST /api/scenes/recall { name }` replays. Scenes stored as JSON in the coordinator state file (no new DB table). Dashboard: "Save scene" button per room card.
...
- **F8** — Creative Features. See *The Innovations* above. Delivery order: (1) Telemetry Lighting; (2) Photo Colour Picker (client-side Canvas API); (3) Intent-First Scenes; (4) Circadian/Sunset Engine (`sunrise`/`spa` crate); (5) Temporal Composer; (6) Reactive Graph; (7) 2D Room Layout (SVG floor plan, builds on F-Rooms).
- **F9** — Light Games. 4×2 colour spot grid as a game board. MVP: Simon Says. Stretch: Colour Match, Disco Party, Hot/Cold (motion sensor driven). Game logic in coordinator Rust for determinism and speed.
- **F10** — Effects Engine. Prerequisite: evaluate SLZB-06 + Z2M latency floor. If viable: `capability_effects` crate with fast-path UDP. Screen sync via `getDisplayMedia` (browser API, no Rust needed for MVP) deferred until fast path confirmed.
