...
use, do you know?"* — and the second half turned out to answer the first.

**What Hue uses, measured off Jon's own dial rather than looked up.** The Hue Tap Dial
emits *two parallel streams* for one rotation. The `dial_rotate_*_step|slow|fast` events
carry only `action_time` and coarse speed buckets — those are what the bindings were on.
Alongside them it emits `brightness_step_up` / `brightness_step_down` carrying the real
figures:

```
"action":"brightness_step_up","action_step_size":44,"action_transition_time":0.04
```

Across 65 events in pi1's z2m log, `action_step_size` runs **8, 14, 20, 26, 32, 38, 44,
50, 56, 62, 68, 74 … 208** — arithmetic, 8 + 6n. **One detent is 8** on the 0–254 scale
(~3%), scaling with rotation speed. `action_transition_time` is **0.04 s** every time.
The configured bindings were ±1 slow and ±10 fast: the slow step was eight times finer
than Hue's own smallest, which is why slow rotation felt dead and speeding up jumped.

**The jitter itself was a different fault, and the first diagnosis of it was wrong.** It
was initially put down to consecutive events reading the same stale brightness — that is
not what happens, because `dispatch_light_command` (`rooms.rs:210`) optimistically applies
each command to the snapshot, so successive dial events do accumulate. The real race is
the echo: `push_lighting_update` (`state.rs:802`) `insert`s whatever a bulb reports with
**no in-flight or timestamp guard**, so a report generated by an *earlier* command lands
300–500 ms later and rolls the snapshot back to an older level. The next dial event, at up
to **4 events/second**, then computes its absolute target from that rolled-back number.
Advance, snap back, advance — the jitter. Correcting this mid-build is recorded because
the wrong version is the more obvious one and would have been repeated.

**The fix removes the read entirely rather than guarding it.** New
`LightAction::BrightnessStep { delta, transition_secs }` maps to z2m's `brightness_step`,
the relative Level Control Step command: the bulb adds the signed delta to whatever level
it is actually at. Nothing in the coordinator needs to know that level, so there is no
value to be stale. This is also what Hue's own system does.

`SwitchActionReport` now carries `step_size` and `transition_secs` (both `Option`, both
`#[serde(default)]`, so an older node's message still decodes — there is a test for that).
The new binding command `brightness_step_relative` takes its **direction** from the
binding's `step_delta` sign and its **magnitude** from the dial's own report, falling back
to the configured magnitude, and finally to Hue's detent of 8 — never to zero, which would
be a command that does nothing.

One further win: the relative path sends **one fan-out for the whole target** instead of
one computed command per device. The absolute path has to go per-device because members
sit at different levels; a relative step is the same instruction for every bulb. On a room
of eight at 4 events/second that is 4 commands rather than 32, which matters given the
Zigbee RF round trip is the dominant latency in this system.

The absolute `brightness_step` is kept, unchanged — it is the right thing for a button
that should step by a fixed amount, and existing bindings use it.

1,308 workspace tests green, clippy clean.
